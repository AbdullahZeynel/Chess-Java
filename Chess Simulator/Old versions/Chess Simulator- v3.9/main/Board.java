...
public class Board extends JPanel {
    ...
    public String fenStartingPos = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
    ...
    public void loadPositionFromFen(String fenString){
        ...
        if (bqr instanceof Rook){
            bqr.hasMoved = !parts[2].equals("q");           //if contains q then it can castles so it hasn't moved so !
        }

        Piece bkr = getPiece(7, 0);
        if (bkr instanceof Rook){
            bkr.hasMoved = !parts[2].equals("k");
        }

        Piece wqr = getPiece(7, 0);
        if (wqr instanceof Rook){
            wqr.hasMoved = !parts[2].equals("Q");
        }

        Piece wkr = getPiece(7, 7);
        if (wkr instanceof Rook){
            wkr.hasMoved = !parts[2].equals("K");
        }
