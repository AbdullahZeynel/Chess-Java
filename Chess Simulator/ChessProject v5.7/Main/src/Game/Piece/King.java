...
//    public boolean isKingChecked;
    ...
    protected boolean canCastle(int col, int row) {
        ...
        ///for the castle move to be applied with white pieces
        ///the rook must be on the 7th row and with black pieces the 0th row
        if (pieceRow == row) {  //if the desired move is on the same row with the king
            if (col == 6) {     // 6 out of 7 with 0 included this is the king's side castle move
                Game.Piece.Piece rook = engine.getPiece(7, row);
                ///get the piece at the rook position if
                /// (rook != null && rook.name.equals("Rook") && rook.isWhite == isWhite)
                ///validate the piece's identity (piece type and color) and it's existence
                    if (rook.isFirstMove == isFirstMove) {              ///check whether the king   and    the rook had moved or not
